...
create table public.members (
  ...
  constraint members_phone_length check (length(phone) <= 80),
  constraint members_address_length check (length(address) <= 500),
  constraint members_profile_note_length check (length(profile_note) <= 2000),
...
create table public.attendance_records (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint attendance_records_note_length check (length(note) <= 1000),
...
create index idx_districts_display_order on public.districts (display_order, name);
create index idx_big_families_display_order on public.big_families (district_id, display_order, name);
create index idx_small_groups_display_order on public.small_groups (district_id, big_family_id, display_order, name);
...
comment on table public.audit_logs is 'Operational audit trail for sensitive app actions.';
